...
<div class="container">
    ...
    <form action="{{route('book_update', $book->id)}}" method="POST" enctype="multipart/form-data">
        ...
        <li>
            <label>
                <input type="checkbox" name="authorID[]" value="{{ $author->id }}"
                    {{ in_array($author->id, $book->authors->pluck('id')->toArray()) ? 'checked' : '' }}>
                {{ $author->name }}
            </label>
        </li>
    @endforeach
        ...
        <div class="mb-3">
            ...
            <textarea name="description" id="bookdescription" value="{{ old('description') }}"
