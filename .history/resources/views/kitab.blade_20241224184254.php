...
<div class="container">
    ...
    <form  action="{{route('book.store')}}" method="POST" enctype="multipart/form-data" >
        ...
        <div class="mb-3">
            ...
            <div class="input-group">
                <input type="text" id="authorsInput" name="author" class="form-control " placeholder="Select authors" readonly>
                <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal"
                    data-bs-target="#addAuthorModal">Add New</button>
            </div>
            <ul id="authorsDropdown" name="author" class="dropdown-menu" style="width: 89%; display: none; margin-top: -1px;">
                @foreach ($authors as $author)
                <li>
                    <label>
                        <input type="checkbox" value="{{ $author->id }}"> {{ $author->name }}
                    </label>
                </li>
                @endforeach
                @error('author')
