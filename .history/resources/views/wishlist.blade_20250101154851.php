                ...
                <div class="row g-0">
                    ...
                    <div class="col-md-9">
                        <div class="card-body p-4">
                            <h3 class="card-title fw-bold text-primary mb-2">{{ $wishlist->book->name }}</h3>
                            <div class="d-flex align-items-center mb-3">
                                @if ($wishlist->book->authors->isNotEmpty())
                            ...
                            <p class="card-text mb-4">{{ \Illuminate\Support\Str::limit($wishlist->book->description, 300, '...') }}</p>
                            <div class="d-flex gap-2 flex-wrap">
                                <a href="{{ route('book', $wishlist->book->id) }}" 
                                   class="btn btn-primary">
                                    <i class="bi bi-book"></i> Read More
                                </a>
                                <form action="{{route('delete_wishlist',$wishlist->book->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button ty class="btn btn-outline-danger">
                                        Remove From Wishlist
                                    </button>

                                </form>
                                
                                @if (($wishlist->book->available_copies) > 1)
