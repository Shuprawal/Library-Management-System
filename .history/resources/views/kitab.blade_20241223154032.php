...
<div class="container">
    ...
    <form method="POST">
            ...
            <div class="dropdown">
                ...
                <ul id="authorDropdown" class="dropdown-menu" style="width: 100%; display: none;">
                    @foreach ($authors as $author)
                        <li><label><input type="checkbox" value="{{ $author->id }}"> {{ $author->name }}</label></li>
                    @endforeach
                    <li class="dropdown-divider"></li>
                    <li><button type="button" class="btn btn-link" data-bs-toggle="modal" data-bs-target="#addAuthorModal">Add New Author</button></li>
                </ul>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="description" class="form-control" id="bookdescription" rows="4" required></textarea>
        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const authorCheckboxes = authorDropdown.querySelectorAll("input[type='checkbox']");

        authorInput.addEventListener("click", function () {
            authorDropdown.style.display = authorDropdown.style.display === "none" ? "block" : "none";
        });
        ...
        authorCheckboxes.forEach(checkbox => {
            checkbox.addEventListener("change", function () {
                ...
                authorInput.value = selectedAuthors.join(", ");
            });
        });

        document.addEventListener("click", function (e) {
            if (!authorInput.contains(e.target) && !authorDropdown.contains(e.target)) {
