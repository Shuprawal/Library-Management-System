    ...
    public function store(Request $request)
{
    $rules = [
        ...
        'available_copy' => ['numeric', 'required', 'min:1'],
    ...
    $messages = [
        ...
        'available_copy.required' => 'The number of available copies is required.',
        'available_copy.numeric' => 'The available copies must be a number.',
        'available_copy.min' => 'The number of available copies must be at least 1.',
    ...
    // try {
        ...
        $book->available_copies = $request->input('available_copy');
        $book->publisherID = $request->input('publisher_id');
        $book->description = $request->input('description');

        // Handle the image upload
        if ($request->hasFile('image')) {
            $imagePath = $request->file('image')->store('book_images', 'public.img');
    ...
    //     return back()->withInput()->with('error', 'An unexpected error occurred while saving the book. Please try again later.');
    // }
}
