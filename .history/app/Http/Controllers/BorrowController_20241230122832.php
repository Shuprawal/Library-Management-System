...
class BorrowController extends Controller
{
    ...
    public function show()
{
    $pendingRequests = Borrow::where('request', 'pending')->get(); // Don't forget to execute the query with get()
    return view('borrowRequest', compact('pendingRequests'));
}

    public function requestAccepted()
    {
        $acceptedRequests = Borrow::where('request', 'accepted')->where( 'status','borrowed')->get(); // Don't forget to execute the query with get()
        return view('borrowRequest', compact('acceptedRequests'));
    }
