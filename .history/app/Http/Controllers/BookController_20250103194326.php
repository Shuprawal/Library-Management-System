    ...
    public function sortList(Request $request)
{
    ...
    public function featureList(){
        ...
        return view('index',compact('featured','totalbook','totalBorrow','totalAuthor','totalUser'));
    }

    public function searchList(Request $request){
        $search=$request->input('search');
        $result=Book::where('name','LIKE',"%{$search}%")->orWhere('isbn','LIKE',"%{$search}%")
        ->orhereHas('authors',function($q)use($search){
            $q->where('name','LIKE',"%{$search}%");
        })
        ->whereHas('publisher',function($q)use($search){
            $q->where('name','LIKE',"%{$search}%");
        })
