...
{
    try {
        // Fetch all books for the given genre ID
        $books = Book::where('genre_id', $genreID)->get(); // Execute the query and get the results

        // Check if the collection is empty
        if ($books->isEmpty()) {
            return redirect()->back()->with('error', 'No books found for this genre.');
        }
       ...
       // \Log::error('Error fetching books by genre: ' . $e->getMessage());
